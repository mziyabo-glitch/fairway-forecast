...
/* =====================================================
   ...
   Search: course name → pick result → current/hourly/daily
   Weather: supports forecast5 list[] + derives hourly/daily
   Extras:
...
(() => {
  ...
  function bestTimeToday(norm) {
    ...
    if (pops.length) {
      ...
      if (minPop >= 0.8 || avgPop >= 0.85) return null; // rain likely all daylight
    }
  ...
  function renderCurrentCard(norm) {
    ...
    // IMPORTANT: no “--” dashes – show empty string if not available
  ...
  function renderHourlyCard(norm) {
    ...
    return `
      <div class="ff-card">
        <div class="ff-card-title">Hourly · next ~48h (preview)</div>
        <div class="ff-table">
  ...
  async function runSearch() {
    ...
    try {
      ...
      // If 1 exact match, auto-select
      if (courses.length === 1) {
        await selectCourse(courses[0]);
        return;
      }

      // Render picker near the top (inside results container)
      const wrapper = document.createElement("div");
      wrapper.innerHTML = renderFavsStrip() + renderCoursePicker(courses, q);
      resultsEl.innerHTML = wrapper.innerHTML;
  ...
  async function updateSuggestions() {
    ...
    try {
      const courses = await fetchCourses(q);
      const names = courses.slice(0, 10).map((c) => c.name || c.course_name || c.club_name).filter(Boolean);

      suggestionsEl.innerHTML = names.map((n) => `<option value="${esc(n)}"></option>`).join("");
    } catch (err) {
      // If upstream rate limited, back off hard
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          // Create a pseudo-course “Your location”
          await selectCourse({
            id: null,
            name: "Your location",
            city: "",
  ...
  function renderAll() {
    ...
    const wrapper = document.createElement("div");
    wrapper.innerHTML = `${favsHtml}${headerHtml}${tabHtml}`;
    resultsEl.innerHTML = wrapper.innerHTML;

    attachFavHandlers(resultsEl);
    attachHeaderHandlers(resultsEl);

    // Keep verdict always in sync
    renderVerdictCard(lastNorm);
  }
  ...
  unitsSelect?.addEventListener("change", async () => {
    // If a course is selected, reload weather in new units
