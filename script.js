...
// --- Playability popover toggle (small help bubble) ---
(function enablePlayabilityPopover(){
  const playCard = document.getElementById("playCardCurrent");
  if (!playCard) return;

  const btn = playCard.querySelector(".infoBtn");
  if (!btn) return;

  function close(){
    playCard.classList.remove("popOpen");
    btn.setAttribute("aria-expanded", "false");
    const pop = playCard.querySelector(".playPopover");
    if (pop) pop.setAttribute("aria-hidden", "true");
  }

  function open(){
    playCard.classList.add("popOpen");
  ...
  btn.addEventListener("click", (e) => {
    e.stopPropagation();
    playCard.classList.contains("popOpen") ? close() : open();
  });

  // click outside closes
  document.addEventListener("click", (e) => {
    if (!playCard.contains(e.target)) close();
  });
